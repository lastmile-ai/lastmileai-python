...
class LastMile(object):
    ...
    def create_dataset(self, name):
        ...
        payload = {"name": name, "type": "FILES", "uploads": [], "metadata": {}}
        headers = self._get_headers()
        resp = requests.post(url, data=json.dumps(payload), headers=headers)
        data = resp.json()
        return data
    ...
    def create_embedding_collection(
        self, name, dataset_id=None, uploads=[], description=None
    ):
    ...
    def update_embedding_collection_status(self, id, ready=False, error=None):
    ...
    def create_model_fork(self, id, name, embedding_collection_id, description=None):
